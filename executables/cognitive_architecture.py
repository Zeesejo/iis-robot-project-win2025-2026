...
class CognitiveArchitecture:
    ...
    def __init__(self, robot_id, table_id, room_id, target_id):
        ...
        self.state_estimator = ParticleFilter(num_particles=100)
    ...
    def _get_lidar_obstacle_avoidance(self, lidar, forward_vel, relaxed=False, robot_pose=None):
        ...
        if robot_pose is not None and self.table_position is not None:
            ...
            if table_dist < table_keepout:
                if table_dist < 0.05:  # very close, small reverse
                    forward_vel = -0.3
                else:  # just stop if near
                    forward_vel = 0.0

                # Compute direction to steer away
                ...
                # Apply gentle angular correction
                avoidance_angular = 2.0 * heading_diff
    ...
    def sense(self):
        ...
        if rgb is not None and self.step_counter % 5 == 0:
    ...
    def act(self, control_commands):
        ...
        if mode in ('search_rotate', 'search_approach', 'navigate'):
            self._stow_arm()
