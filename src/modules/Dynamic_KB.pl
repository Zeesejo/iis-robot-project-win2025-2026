...
% Sensors
% -----------------------------
sensor(rgbd_camera_link, camera).
sensor(lidar_link, lidar).
sensor(imu_link, imu).
sensor(joint_state_sensor, joint_states).

has_sensor(robot, camera_rgb).
has_sensor(robot, camera_depth).
has_sensor(robot, camera_mask).
has_sensor(robot, lidar).
has_sensor(robot, imu).
has_sensor(robot, joint_states).

sensor_link_for_robot(camera_rgb, rgbd_camera_link).
sensor_link_for_robot(camera_depth, rgbd_camera_link).
sensor_link_for_robot(camera_mask, rgbd_camera_link).
...
position(target, -2.4598, 3.8106, 0.06).
...
% Sensor logic
% -----------------------------
perception_sensor_link(Link) :- sensor(Link, camera).
perception_sensor_link(Link) :- sensor(Link, lidar).
...
visible_to_sensor(Link, Obj) :-
    ...
    position(Obj,Ox,Oy,_),
    distance(Rx,Ry,Ox,Oy,D),
    D =< 5.0.

detectable_objects(Obj) :-
    has_sensor(robot, SensorType),
    sensor_link_for_robot(SensorType, Link),
    visible_to_sensor(Link, Obj).

% Return a unique list of all detectable objects (removes duplicates)
